...
public class PlayerController : MonoBehaviour
{
    ...
    const int FALL_COUNT_UNITY = 120;
    ...
    public void SetLogicalInput(LogicalInput referense)
    {
        _logicalInput = referense;
    }
 ...
 bool Rotate(bool is_right) 
    ...
    void Settle() 
    {
        ...
        bool is_set1 = boardController.Settle( CalcChildPuyoPos
            (_position,_rotate), (int)_puyoControllers[1].GetPuyoType());
        //Debug.Assert(is_set1);
    }

    bool Fall(bool is_false) 
    {

        _fallCount -= is_false ? FALL_COUNT_FAST_SPD : FALL_COUNT_SPD;
        while (_fallCount < 0) 
        {
...
/*static readonly KeyCode[] Key_code_tbl = new KeyCode[(int)LogicalInput.Key.MAX] {
    ...
    */
    void Update()
    {
        ...
        //if (!_animationController.Update(Time.deltaTime)) { Control(); }
 ...
 private void FixedUpdate()
    {
        ...
        Control();
        Vector3 dy = Vector3.up * (float)_fallCount / (float)FALL_COUNT_UNITY;
        float anim_rate = _animationController.GetNormalized();
        _puyoControllers[0].SetPos(Interpolate(_position, RotState.Invalid, _last_position, RotState.Invalid, anim_rate));
        _puyoControllers[1].SetPos(Interpolate(_position, _rotate, _last_position, _last_rotate, anim_rate));
    }
